...
import useUpdateUser, { useCheckPassword } from "../../../utils/react-query-hooks/auth/useUpdateUser";
...
export default function SettingsAccountPage() {
  ...
  // Reset password fields when canceling password change
  ...
  const checkPassword = useCheckPassword(
    ...
    (response) => {
      if (response?.data?.isValid) {
        ...
        updateUser.mutate(formDataToUpdate);
      } else {
        setErrorMessage("Current password is incorrect");
        setIsSubmitting(false);
      }
    },
    // On password check error
    (error) => {
      setErrorMessage(error?.response?.data?.error?.message || "Failed to verify password");
      setIsSubmitting(false);
    },
  );

  // Store form data temporarily during password check
  ...
  const onSubmit = (data) => {
    ...
    // Prepare query object with changed fields only
    const query = {};
    ...
    if (showPasswordFields) {
      ...
      // Store the query for later use after password check
      formDataToUpdate = query;

      // First verify the current password
      checkPassword.mutate(data.password);
      return;
    }

    // If not changing password, update directly
