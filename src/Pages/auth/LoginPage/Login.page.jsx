...
export default function LoginPage() {
  ...
  // Check if user has incomplete listing data and was redirected from addList page
  useEffect(() => {
    // Check localStorage for listing data
    const listingData = localStorage.getItem("listingFormData");

    // Check if the user was redirected from the addList page
    const returnTo = location.state?.returnTo;
    const fromListingFlow = returnTo === "/addList" || location.state?.listingData;

    // Only show the listing message if both conditions are true
    if (listingData && fromListingFlow) {
  ...
  const onSuccess = (data) => {
    ...
    // Only redirect to addList if user came from there specifically
    if (returnTo && returnTo === "/addList" && hasListingData) {
      navigate(returnTo);
    } else {
        ...
        <div className="max-w-md mx-auto h-full flex py-8 sm:py-12 md:py-20 px-4">
          ...
          >
            <div className="px-4 sm:px-6">
              <h1 id="login-heading" className="text-xl sm:text-2xl font-medium mb-2">
                Login to continue
              </h1>
            ...
            <div className="text-sm mt-2 font-normal text-gray-600">
              ...
              <Link to="/auth/register" className="text-primaryA0 hover:underline transition-colors">
                Register now
              </Link>
            </div>
          </form>
