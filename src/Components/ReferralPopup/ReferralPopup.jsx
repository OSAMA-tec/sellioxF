...
const ReferralPopup = () => {
  ...
  useEffect(() => {
    // Make sure Redux state is initialized before making decisions
    if (!isInitialized) return;

    // Don't show popup if user is logged in - strict check to ensure user object is valid
    if (user && user._id) {
      console.log("User is authenticated, hiding referral popup");
      setIsVisible(false);
      return;
    }
    ...
    // Don't show popup on referral-related pages that require authentication
    ...
    if (!hasSeenPopup) {
      ...
      const timer = setTimeout(() => {
        // Double-check user state before showing
        if (!user || !user._id) {
          setIsVisible(true);
        }
      }, delay);
    ...
    >
      ...
      >
        ...
        <div className="text-white p-6 text-center referral-popup-header">
          ...
          <h2 id="referral-popup-title" className="text-2xl font-bold">
        ...
        <div className="p-6 referral-popup-content">
          <p className="text-gray-700 mb-5 text-center">Join our referral program and earn tickets to our monthly $1,000 prize draw!</p>
          ...
          <div className="space-y-5 mb-6">
            <div className="flex items-start referral-popup-benefit">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Get 1 ticket when someone signs up with your code, 5 more when they create a listing
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start referral-popup-benefit">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Every ticket is an entry into our $1,000 monthly prize draw - more tickets, more chances!
                </p>
              </div>
            </div>
          </div>
          ...
          <p className="text-sm text-gray-500 text-center mt-5">
            ...
            <button onClick={() => navigate("/auth/login")} className="text-primaryA0 hover:underline font-medium">
              Sign in
            </button>
          </p>

          <p className="text-xs text-gray-400 text-center mt-3">You can access the referral program anytime from your account menu</p>
        </div>
      </div>
    </div>
  );
